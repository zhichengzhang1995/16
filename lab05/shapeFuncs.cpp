...
bool boxesApproxEqual(Box b1, Box b2, double tolerance) {
  ...
  if (!approxEqual(b1.width, b2.width))
	  return false;
  else if (!approxEqual(b1.height, b2.height))
	  return false;
  else if (!approxEqual(b1.ul.x, b2.ul.x))
	  return false;
  else if (!approxEqual(b1.ul.y, b2.ul.y))
	  return false;
  else 
	  return true;
